...
class Stopper(object):
    ...
    def scan_callback(self, scan_msg):
        ...
        for i in range(320):
            if scan_msg.ranges[i] > self.min_dist_from_obstacle or (math.isnan(
                    scan_msg.ranges[i]) and i > 0 and math.isnan(scan_msg.ranges[i - 1])):
                left = i
            if scan_msg.ranges[639 - i] > self.min_dist_from_obstacle or (
                    math.isnan(scan_msg.ranges[639 - i]) and math.isnan(scan_msg.ranges[(639 - i) - 1])):
                right = 639 - i
    ...
    def rotate(self):
        # rot = Twist()
        # rot.angular.z = self.direction * self.round * (3.14159265359 / 180)
